...
sudo rm -r apache-flume-1.9.0-bin
sudo rm apache-flume-1.9.0-bin.tar 
...
sudo mv /Users/neftalivaldez/Downloads/kafka_2.13-2.8.0.tar /usr/local/kafka
tar -xzvf kafka_2.13-2.8.0
cd kafka
sudo mv * /usr/local/kafka
cd ..
sudo rm -R kafka_2.13-2.8.0
...
# <- Es lo mismo 
#sudo ./bin/flume-ng agent --conf ${FLUME_HOME}/conf/  -f ${FLUME_HOME}/conf/spool-kafka-flume-conf.properties  -n agent -Dflume.root.logger=INFO,console
